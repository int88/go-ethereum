...
// Process返回在处理过程中积累的receipts和logs，并且返回在处理过程中使用的gas数量
// 如果任何的transactions执行失败，因为gas不足，则会返回error
...
// ApplyTransaction试着应用一个transaction到给定的state database并且使用输入参数作为
// environment，它为transaction返回receipt，使用的gas以及error如果transaction执行失败的话
// 表明block非法
func ApplyTransaction(config *params.ChainConfig, bc ChainContext, author *common.Address, gp *GasPool, statedb *state.StateDB, header *types.Header, tx *types.Transaction, usedGas *uint64, cfg vm.Config) (*types.Receipt, error) {
	log.Info("ApplyTransaction is being called")
	msg, err := tx.AsMessage(types.MakeSigner(config, header.Number), header.BaseFee)
	if err != nil {
		return nil, err
	}
	// Create a new context to be used in the EVM environment
	// 创建一个新的context，在EVM environment的上下文中
