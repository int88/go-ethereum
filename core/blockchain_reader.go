...
// CurrentHeader获取当前的canonical chain的head header，header从HeaderChain的internal cache
// 中获取
...
// CurrentBlock获取canonical chain的current head block，block从blockchain的internal cache中
// 获取
...
// GetHeadersFrom返回headers的contiguous segment，以rlp的形式，从给定的number往后
...
// caching it if found.
// GetBlock从数据库通过hash以number获取一个block，缓存它如果找到的话
...
// GetReceiptsByHash获取一个给定的block的所有transactions的receipts
...
// GetTd获取一个block的total difficrulty，在canonical chain的database中，通过
// hash以及number，缓存它，如果找到的话
func (bc *BlockChain) GetTd(hash common.Hash, number uint64) *big.Int {
	// 最终从header chain中获取到TD
